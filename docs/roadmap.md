...
- ~~**1 fluxo E2E** via `docker compose`: register → produto → stock → cart → checkout → poll até `CONFIRMED` ou `CANCELLED`~~ **Step 4 concluído** (`scripts/e2e-checkout.sh`, `yarn test:e2e`)
...
| 7. Test coverage | **Concluída** — pacotes `@ecommerce/auth-middleware` (4), `@ecommerce/messaging` (8), `@ecommerce/event-contracts` (19); gateway smoke tests; gaps unitários finos; runner raiz `yarn test` + CI GitHub Actions; E2E compose `yarn test:e2e` (`scripts/e2e-checkout.sh`) |
| 8. Polish | Pendente |

...
| 4. Orders + saga parcial | **Em progresso** — `orders` Step 1 (criar pedido, `order.created`, consumers `stock.*`); **`inventory` Step 2** (reserva em `order.created`); falta compose/gateway |
